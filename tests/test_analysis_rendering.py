...
def test_synthetic_panel_writes_watermarked_composite_and_exact_slide_exports(
    ...
    composite = tmp_path / "figures" / "manuscript" / "panel2_multifeature_decoding.png"
    slide = (
        tmp_path
        / "figures"
        / "slides"
    ...
    assert (slide.parent / "panel2_multifeature_decoding.txt").exists()
...
def test_panel3_writes_manuscript_composite_and_captions(tmp_path: Path):
    ...
    composite = tmp_path / "figures" / "manuscript" / "panel3_network_dynamics.png"
    slide_caption = (
        tmp_path / "figures" / "slides" / "panel3_network_dynamics" / "panel3_network_dynamics.txt"
    )
    assert composite in outputs
    assert composite.with_suffix(".json").exists()
    assert composite.with_suffix(".txt").exists()
...
def test_panel4_writes_manuscript_composite_and_captions(tmp_path: Path):
    outputs = render_panels(
        panel="panel4", analysis_id=None, analysis_root=None, reports_root=tmp_path
    )
    composite = tmp_path / "figures" / "manuscript" / "panel4_network_state_attribution.png"
    assert composite in outputs
    assert composite.with_suffix(".json").exists()
    assert composite.with_suffix(".txt").exists()
